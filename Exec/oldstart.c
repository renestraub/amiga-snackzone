...
**   16-Jun-91  RS   Findet jetzt MemoryListe auch unter 2.0.              **
**                                                                         **
****************************************************************************/
...
char ident[]         = "$VER: Exec V4.3 (" __DATE__ ") by Christian A. Weber";
...
local BOOL SaveChipRAM(void)
{
	...
	if(NewOS)
	{
		mrh=(struct MemHeader *)FindName(&SysBase->MemList,"chip memory");
	}
	else
	{
		mrh=(struct MemHeader *)FindName(&SysBase->MemList,"Chip Memory");
...
BYTE GetDriveVars(char *drive)
{
	...
	rn = (struct RootNode *)ArpBase->DosRootNode;
	...
	do
	{
		...
		{
			name=(UBYTE *)BADDR(ldn->dn_Name)+1;
			i=0;

			for(;;)
			{
				if((name[i]&0xDF) != (drive[i++]&0xDF)) goto end;
				if(name[i]==0) {break;}
			}

			found=TRUE;

			fs=(struct FileSysStartupMsg *)BADDR(ldn->dn_Startup);
